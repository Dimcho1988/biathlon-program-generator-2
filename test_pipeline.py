...
def test_full_pipeline_generates_real_plan():
    ...
    assert analysis["decision_snapshot"]["algorithm_version"] == "streamlit-demo-0.6.0"
...
def test_readiness_curve_advances_to_analysis_date_after_days_without_training():
    bundle = generate_demo_bundle(history_days=90)
    last_activity = pd.to_datetime(bundle["activities"]["date"]).max().normalize()
    analysis_date = (last_activity + pd.Timedelta(days=3)).date()

    analysis = analyze_athlete(bundle, "A", as_of=analysis_date, generate_plan=False)
    readiness = analysis["readiness_history"]

    assert pd.to_datetime(readiness["date"]).max().date() == analysis_date
    recovery_only = readiness.loc[pd.to_datetime(readiness["date"]) > last_activity]
    assert not recovery_only.empty
    assert recovery_only["impulse"].eq(0.0).all()
