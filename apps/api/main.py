...
def response_history(period_start: date | None = None, period_end: date | None = None,
    ...
    try:
        return response_service.history(_repository(), alias, period_start, period_end)
...
def _save_response(kind, body, authorization, athlete_alias, actor):
    ...
    try:
        return response_service.save_report(_repository(), alias, kind, body, actor)
    except PersistentStoreFailure as exc:
        raise HTTPException(503, "Response storage is unavailable") from exc


@app.put("/api/v2/athlete/response/daily")
...
@app.put("/api/v2/athlete/response/session")
...
@app.put("/api/v2/athlete/response/block")
...
@app.put("/api/v2/athlete/response/test")
