...
describe("training-status-v1 contract", () => {
  ...
  it("rejects a Tref value outside the approved zone bounds", () => {
    const zones = trainingStatusFixture.zones.map((zone) => zone.zone === "Z5" ? { ...zone, tref_min: 21 } : zone);
    expect(() => parseTrainingStatus({ ...trainingStatusFixture, zones })).toThrow(/Tref.*граници/);
  });
