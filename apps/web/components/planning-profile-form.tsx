...
export function PlanningProfileForm({profile,managementProfile,accentPreferences,planningCalendar,notice}: {
 ...
 <ManagementProfileEditor key={String(managementProfile?.revision??0)} initialProfile={managementProfile??{configured:false,profile:null,revision:0}} today={managementProfile?.today??new Date().toISOString().slice(0,10)} legacy={profile} legacyAccents={accentPreferences}/>
