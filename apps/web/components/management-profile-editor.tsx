"use client";
import { RaceDurationEstimate } from "./race-duration-estimate";
...
import { trainingDays, defaultManagementProfile, defaultPlanningControls, defaultLoadProgression, parseManagementProfile, parseManagementProfileResponse, type ManagementProfile, type ManagementProfileResponse } from "../lib/training-management";
import { LoadProgressionEditor } from "./load-progression-editor";
...
export function ManagementProfileEditor({ initialProfile, today, canEdit = true, legacy, legacyAccents, calendar, outlook }: { calendar?: PlanningCalendarResponse; outlook?: PlanProjection | null; initialProfile: ManagementProfileResponse; today: string; canEdit?: boolean; legacy?: PlanningProfile|null; legacyAccents?: MesocycleAccentPreferencesResponse }) {
  ...
  const [profile, setProfile] = useState<ManagementProfile>(()=>{
    const source=initialProfile.profile??defaultManagementProfile(today);
    const p={...source,load_progression:source.load_progression??defaultLoadProgression()};
  ...
  return <section className="management-panel management-profile" id="basic-profile">
    ...
    {saved.configured&&!saved.profile?.load_progression&&<p className="management-notice">Добавено е управление на дългосрочния прираст. Прегледай настройките в стъпка 3; те влизат в сила след запазване на профила.</p>}
    ...
    {step===1&&<><h2>1. Спорт и цел</h2><p className="management-muted">Задължителни: спорт, средство, дисциплина и период на подготовката.</p><div className="management-form-grid">
      ...
      <RaceDurationEstimate key={`${profile.sport}:${profile.discipline}:${profile.race_duration_min}`} sport={profile.sport} discipline={profile.discipline} fallback={profile.race_duration_min}/>
      <label>Продължителност на старта, мин · ако моделът няма оценка<input type="number" min=".1" max="1440" step=".1" value={profile.race_duration_min??""} onChange={e=>update("race_duration_min",optional(e.target.value))}/></label>
    ...
    {step===3&&<><PlanningControlsEditor profile={profile} onChange={setProfile} stage="cycles" today={today}/><LoadProgressionEditor profile={profile} onChange={setProfile}/></>}
