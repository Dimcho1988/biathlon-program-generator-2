...
import { bandFor, indexNumber, lineSegments, type TrainabilityHistory } from "../lib/trainability";
...
export function TrainabilityHistoryView({ history }: { history: TrainabilityHistory }) {
  ...
  return <div className="trainability-content">
    <section className="index-chart-panel" aria-label="Динамика на индекса">
      ...
      </svg> : <p className="detail-empty">{enabled.length === 0 ? "Изберете поне една линия." : "Няма валидни индекси за избрания период и спорт. Нужни са активност от поне 7 минути и сумарно HRmod време и разпределено скоростно време по поне 7 минути за Z1–Z4 и общия диапазон, или 5 минути за Z5."}</p>}
      <p className="index-note">Индекс = HRmod (%HRmax) / Vflat (km/h). По-ниската стойност означава по-малък относителен пулс спрямо Vflat. Сравнявайте всяка зона със същата зона. Линиите прекъсват при невалиден индекс; различни спортове и настройки се разглеждат отделно.</p>
