...
export function TrainabilityHistoryView({ history }: { history: TrainabilityHistory }) {
  ...
  return <div className="trainability-content">
    <section className="index-chart-panel" aria-label="Динамика на индекса">
      ...
      </svg> : <p className="detail-empty">{enabled.length === 0 ? "Изберете поне една линия." : "Няма валидни индекси за избрания период и спорт. Нужни са активност от поне 7 минути и поне 60 секунди HRmod и разпределени скорости в съответния диапазон."}</p>}
      <p className="index-note">По-ниската стойност означава по-малко пулс спрямо Vflat. Сравнявайте всяка зона със същата зона. Линиите прекъсват при невалиден индекс; различни спортове и настройки се разглеждат отделно.</p>
