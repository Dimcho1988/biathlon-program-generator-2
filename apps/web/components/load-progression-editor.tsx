...
export function LoadProgressionEditor({profile, onChange}: {profile: ManagementProfile; onChange: (p: ManagementProfile) => void}) {
  ...
  return <section aria-label="Прираст на натоварването">
    ...
    {value.enabled && <>
      ...
      <p className="management-muted">Z5 се приравнява към долната граница с 5% за удар; Z1–Z4 остават с 3% към горната. Годишният темп се натрупва според календарното време: за всички аеробни компоненти в общата подготовка, след това според специалните приоритети. Разтоварването намалява седмичната доза, без да занулява развитието на опората.</p>
      ...
      <details><summary>Експертни граници и настройка по периоди</summary>
        <p>Начални треньорски настройки за приравнения обем. Измерената история се запазва и извън границите.</p>
        <table><thead><tr><th>Компонент</th><th>Приравнени минути / седмица</th></tr></thead><tbody>{[["Z1","240–840"],["Z2","60–300"],["Z3","30–120"],["Z4","10–40"],["Z5","5–30"]].map(([z,b])=><tr key={z}><td>{z}</td><td>{b}</td></tr>)}</tbody></table>
