...
interface Week { volume_budget_minutes: number | null; cycle?: Record<string, unknown>; start_date: string; end_date: string; accents: string[]; phases: string[]; components: Partial<Record<Component, { target_period_q: number | null; target_index_7_40: number | null }>> }
function outlookWeeks(plan?: PlanProjection): Week[] {
  ...
  return outlook.weeks.filter(isRecord).map(w => ({ volume_budget_minutes: numeric(w.volume_budget_minutes), cycle: isRecord(w.cycle) ? w.cycle : undefined, start_date: label(w.start_date), end_date: label(w.end_date), accents: Array.isArray(w.accents) ? w.accents.map(label) : [], phases: Array.isArray(w.phases) ? w.phases.map(label) : [], components: Object.fromEntries(COMPONENTS.map(z => {
    ...
    return [z, { target_period_q: numeric(c.target_period_q), target_index_7_40: numeric(c.target_index_7_40) }];
  })) }));
}
...
export function TrainingPlanOverview({ plan, outcomes, today, stale, currentProfileRevision, volumeContext, sessions, sessionStatus = "Няма актуални съставени сесии. Подготви или обнови седмичната програма." }: { plan?: PlanProjection; outcomes: PlanOutcome[]; today: string; stale?: boolean; currentProfileRevision?: number; volumeContext?: Record<string, unknown>; sessions?: PlanningDraft; sessionStatus?: string }) {
  ...
  return <section className="management-overview" aria-label="Дългосрочна подготовка">
    ...
    {volumeContext && <section className="management-panel"><h2>Обем на подготовката</h2>
      ...
      <div className="management-metrics"><div><small>Средна продължителност от историята</small><strong>{numeric(volumeContext.historical_training_weekly_minutes) === null ? "—" : durationHms(Number(volumeContext.historical_training_weekly_minutes))}</strong><span>време за тренировки за 7 дни</span></div><TimeAvailability context={volumeContext}/></div>
      ...
      <LoadMeasureExplanation plan={plan}/>
    </section>}
    ...
    <section className="management-panel"><h2>Динамика по зони</h2><p className="management-muted">Целеви товар според периода, акцентите и разтоварването. Дневната доза се уточнява отделно според готовността.</p>
      ...
      {weeks.length === 0 ? <p>Няма бъдещи седмици за показване. Провери периода на подготовката в профила.</p> : <>
        <div className="management-view-switch" role="group" aria-label="Показател за динамиката">
        ...
        <details className="management-detail"><summary>Как се изчислява динамиката?</summary><p>Планът използва избраната опора: историята между експертните граници, долната граница при по-ниска история и горната при по-висока. Измерените данни остават отделни. Липсващо или нулево измерено натоварване не разрешава автоматично въвеждане на компонент.</p><p>Прирастът описва развитието на опората във времето. В общата подготовка се развиват всички аеробни компоненти; след това темпът следва специалните приоритети. Обичайната седмична вълна разпределя средния обем на целия мезоцикъл, включително разтоварването. Тейпърът и изрично намалените вълни остават с по-малък обем.</p><p>Конкретните тренировки се проверяват отделно според 7/40, възстановяването, методите и наличното време. Целите не са гарантирано изпълнение и не се записват като реална история. За силата няма автоматичен годишен прираст.</p></details>
      </>}
    </section>
    {weeks.length > 0 && <MicrocycleVolumes weeks={weeks} sessions={sessions} status={sessionStatus} metric={metric}/> }
