...
interface Week { volume_budget_minutes: number | null; cycle?: Record<string, unknown>; start_date: string; end_date: string; accents: string[]; phases: string[]; components: Partial<Record<Component, { target_weekly_q: number | null; target_period_effective: number | null; target_weekly_effective: number | null; target_index_7_40: number | null }>> }
function outlookWeeks(plan?: PlanProjection): Week[] {
  ...
  return outlook.weeks.filter(isRecord).map(w => ({ volume_budget_minutes: numeric(w.volume_budget_minutes), cycle: isRecord(w.cycle) ? w.cycle : undefined, start_date: label(w.start_date), end_date: label(w.end_date), accents: Array.isArray(w.accents) ? w.accents.map(label) : [], phases: Array.isArray(w.phases) ? w.phases.map(label) : [], components: Object.fromEntries(COMPONENTS.map(z => {
    ...
    return [z, { target_weekly_q: numeric(c.target_weekly_q), target_period_effective: numeric(c.target_period_effective), target_weekly_effective: numeric(c.target_weekly_effective), target_index_7_40: numeric(c.target_index_7_40) }];
  })) }));
}
...
export function TrainingPlanOverview({ plan, outcomes, today, stale, currentProfileRevision, volumeContext, sessions, sessionStatus = "Няма актуални съставени сесии. Подготви или обнови седмичната програма." }: { plan?: PlanProjection; outcomes: PlanOutcome[]; today: string; stale?: boolean; currentProfileRevision?: number; volumeContext?: Record<string, unknown>; sessions?: PlanningDraft; sessionStatus?: string }) {
  const [metric, setMetric] = useState<"target_weekly_q" | "target_period_effective" | "target_weekly_effective" | "target_index_7_40">("target_period_effective");
  ...
  return <section className="management-overview" aria-label="Дългосрочна подготовка">
    ...
    <section className="management-panel"><h2>Динамика по зони</h2><p className="management-muted">Целеви товар според периода, акцентите и разтоварването. Дневната доза се уточнява отделно според готовността.</p>
      ...
      {weeks.length === 0 ? <p>Няма бъдещи седмици за показване. Провери периода на подготовката в профила.</p> : <>
        <div className="management-view-switch" role="group" aria-label="Мярка на динамиката"><button type="button" aria-pressed={metric === "target_weekly_q"} onClick={() => setMetric("target_weekly_q")}>Прираст и цели Q / 7 дни</button><button type="button" aria-pressed={metric === "target_period_effective"} onClick={() => setMetric("target_period_effective")}>Обем за микроцикъла</button><button type="button" aria-pressed={metric === "target_weekly_effective"} onClick={() => setMetric("target_weekly_effective")}>Товар за 7 дни</button><button type="button" aria-pressed={metric === "target_index_7_40"} onClick={() => setMetric("target_index_7_40")}>Целеви 7/40</button></div>
        ...
        <p className="management-muted">{metric === "target_weekly_q" ? "Директно приравнено време Q за 7 дни по устойчивата база и акцентите, преди преливането. Това са цели преди ограниченията на ежедневните сесии." : metric === "target_period_effective" ? "Целеви ефективен обем E за точните дати на микроцикъла, в приравнени минути. Първият и последният отрязък може да са непълни. Това не са часове от конкретни сесии. STR е отделен компонент." : metric === "target_weekly_effective" ? "Приравнени минути за 7 дни — различни от часовете тренировка и измереното време в зона. STR е отделен компонент." : "Цел спрямо текущата 40-дневна база. Това не е прогноза на действителния бъдещ 7/40; базата ще се обновява с изпълнението."}</p>
        <div className="management-chart-wrap"><svg className="management-outlook-chart" viewBox="0 0 900 305" role="img" aria-label={metric === "target_weekly_q" ? "Целеви директен обем Q по микроцикли" : metric === "target_period_effective" ? "Целеви обем по микроцикли" : metric === "target_weekly_effective" ? "Целеви приравнен товар по седмици" : "Целеви 7/40 спрямо текущата база"}>
        ...
        {current && <div className="management-week-context">{current.cycle && <p><strong>{label(current.cycle.name)}</strong> · {label(current.cycle.kind) === "STRESS" ? "Стресов микроцикъл" : label(current.cycle.kind) === "RECOVERY" ? "Разтоварване" : "Тренировъчен блок"} · седмица {label(current.cycle.week)}</p>}<p><strong>{current.phases.map(p => PHASE_LABELS[p] ?? p).join(" → ")}</strong> · {current.cycle?.focus_role === "RECOVERY_SUPPORT" ? "Допълващо поддържане" : "Акценти"}: {current.accents.join(", ") || "няма"}</p>
          ...
          <div className="management-table-wrap"><table><thead><tr><th>Компонент</th><th>Обем E за избрания период, мин</th><th>Целеви товар / 7 дни</th><th>Целеви 7/40</th></tr></thead><tbody>{COMPONENTS.map(z => <tr key={z}><th>{z}</th><td>{display(current.components[z]?.target_period_effective ?? null)}</td><td>{display(current.components[z]?.target_weekly_effective ?? null)}</td><td>{display(current.components[z]?.target_index_7_40 ?? null)}</td></tr>)}</tbody></table></div>
        </div>}
        <details className="management-detail"><summary>Как се изчислява динамиката?</summary><p>Акцентите са на мезоцикъла. При разтоварване товарът на предишните акценти е до 65% от текущата 40-дневна основа, допълващият компонент — до 90%, а сумарният аеробен товар — до 78%. Това са начални треньорски ограничения, не измерване на физиологичния стрес. Разливането по зони и Recovery могат допълнително да намалят или отложат задачата.</p><p>Натоварващата вълна повишава водещите акценти. Поддържаните компоненти не получават увеличението над 1. Прирастът се задава в директни приравнени минути Q от устойчива опора. Целите за E и 7/40 са отделни ограничения; разтоварването не се наваксва. Отделно зададен микроцикъл използва собствения си индекс. Прилагат се правилата за вработване, преход и тейпър. Дневната готовност може да ограничи изпълнението, без да променя показаната цел.</p><p>Седмиците следват началото на мезоцикъла; първата може да е непълна. Показана е средната цел в отрязъка, включително при смяна на периода. Историческата база остава фиксирана към датата на изчисление; бъдещи тренировки не я увеличават изкуствено.</p><p>7/40 = (B50 + целеви седмичен товар / 7) / (B50 + C40). Празна стойност означава недостатъчна реална история. Лагерът е календарен контекст и не разрешава автоматично увеличение на товара.</p></details>
      </>}
    </section>
    {weeks.length > 0 && <MicrocycleVolumes weeks={weeks} sessions={sessions} status={sessionStatus}/> }
    <PlanComparison plan={sessions ?? plan} outcomes={outcomes} />
