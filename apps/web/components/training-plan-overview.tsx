...
export function TrainingPlanOverview({ plan, outcomes, today, stale, currentProfileRevision, volumeContext }: { plan?: PlanProjection; outcomes: PlanOutcome[]; today: string; stale?: boolean; currentProfileRevision?: number; volumeContext?: Record<string, unknown> }) {
  ...
  return <section className="management-overview" aria-label="Дългосрочна подготовка">
    {volumeContext && <section className="management-panel"><h2>Обем на подготовката</h2>
      ...
      <div className="management-metrics"><div><small>Историческа основа за програмата</small><strong>{display(numeric(volumeContext.historical_training_weekly_minutes) === null ? null : Number(volumeContext.historical_training_weekly_minutes)/60)} ч</strong><span>средно за 7 дни</span></div><div><small>Налично време</small><strong>{display(Number(volumeContext.available_weekly_minutes)/60)} ч</strong><span>за 7 дни</span></div><div><small>Бюджет за избраната седмица</small><strong>{display(current?.volume_budget_minutes == null ? null : current.volume_budget_minutes/60)} ч</strong><span>след вълна, календар и време</span></div></div>
      {Number(volumeContext.available_weekly_minutes) < Number(volumeContext.historical_training_weekly_minutes) && <p className="management-notice">Записаното свободно време е по-малко от историческия обем и ограничава програмата. <Link href="/planning">Провери дните и минутите в профила →</Link></p>}
      <p className="management-muted">Общата история определя основата на часовете; при изключена сила нейното време се отделя. Конкретната доза се проверява по средство, капацитет, 7/40 и Recovery. Бюджетът не е обещан обем.</p>
    </section>}
    ...
    <section className="management-panel"><h2>Динамика по зони</h2><p className="management-muted">Целеви товар според периода, акцентите и разтоварването. Дневната доза се уточнява отделно според готовността.</p>
      {isRecord(plan?.long_term) && plan.long_term.limited === true && <p className="management-notice">Ориентировъчни цели при ограничена история. Те не разрешават започване на програмата.</p>}
