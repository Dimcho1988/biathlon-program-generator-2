  ...
  return <>
    ...
    <section className="activity-detail-layout">
      <div className="activity-detail-main">
        ...
        {series ? <div className="canonical-charts"><TimelineChart title="Пулс" series={series.series} channels={[{ key: "hr_bpm", label: "HR", color: "#df4e5b", unit: "bpm" }]} /><TimelineChart title={isRun ? "Скорост / темпо" : "Скорост"} series={series.series} channels={[{ key: "speed_kmh", label: "Скорост", color: "#07888d", unit: "km/h" }]} /><TimelineChart title="Височина" series={series.series} channels={[{ key: "altitude_m", label: "Височина", color: "#6558d5", unit: "m" }]} /><TimelineChart title="Наклон" series={series.series} channels={[{ key: "grade_pct", label: "Наклон", color: "#a76400", unit: "%" }]} emptyMessage={activity.shadow_available ? <>Наклонът не е част от canonical серията за тази активност. Наличният provider/изгладен наклон е показан само в <Link href={`/activities/${activity.activity_ref}/shadow`}>Experimental анализа</Link>.</> : "За тази активност няма наличен канал за наклон."} /></div> : <section className="canonical-chart"><h3>Времеви серии</h3><p>{seriesUnavailable ? "Графиките временно не са заредени. Canonical summary остава достъпен." : "За тази активност няма съхранени графични канали."}</p></section>}
