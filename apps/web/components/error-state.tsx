...
import { API_RATE_LIMIT_MESSAGE, renderWakeHref } from "../lib/api-wake";
...
export function ErrorState({ message, integrationActions = false, connectAvailable = true, profileSelectionAvailable = false, retryAvailable = false, retryHref = "/", refreshAvailable = true, notice }: {
  ...
  const apiRateLimited = message === API_RATE_LIMIT_MESSAGE;
  const needsWake = apiWakeTimedOut || apiRateLimited;
  const wakeHref = needsWake ? renderWakeHref(process.env.ONFLOWS_API_BASE_URL) : undefined;
  return (
    <main className="state-page" role="alert">
      ...
      {(integrationActions || profileSelectionAvailable || retryAvailable) && <div className="integration-actions">
        ...
        {retryAvailable && !needsWake && <a className="action-button" href={retryHref}>Опитай отново</a>}
        {integrationActions && refreshAvailable && !needsWake && <SyncActionForm label="Обнови реалните данни" />}
      </div>}
      ...
      {needsWake && <ApiWakeRetry wakeHref={wakeHref} retryHref={retryHref} automatic={!apiRateLimited || Boolean(wakeHref)} />}
      {needsWake && <p className="state-help">Първият старт след пауза може да отнеме около минута. Не е нужно да свързваш профила отново.</p>}
    </main>
  );
}
