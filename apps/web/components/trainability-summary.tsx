...
export function TrainabilitySummary({ index }: { index: TrainabilityIndex | null }) {
  ...
  return <section className="trainability-summary" aria-label="Индекси на активността">
    <div className="section-heading"><div><p className="section-kicker">HRmod (%HRmax) / Vflat (km/h)</p><h2>Индекс на тренираност</h2></div><Link href="/trainability">Динамика на индекса →</Link></div>
    <p>По-ниска стойност: по-малък процент от максималния пулс спрямо разпределената еквивалентна скорост. Използва се процентното число, например 78 / 20 = 3,90. Скоростите се разпределят по ранг; не са измерени непременно в същите секунди като пулса.</p>
    ...
    <p className="index-note">Минимум 7 минути за активността. Сумарното HRmod време и разпределеното скоростно време трябва поотделно да са поне 7 минути за Z1–Z4 и общия диапазон, или 5 минути за Z5. Общият индекс се изчислява отделно за 75–92% HRmax. Сравнявайте при сходен спорт, техника, екипировка и условия.</p>
  </section>;
}
