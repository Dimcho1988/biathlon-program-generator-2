...
export function parseIndexBand(v: unknown): IndexBand {
  if (!isRecord(v) || typeof v.name !== "string" || typeof v.valid !== "boolean" ||
    ...
    (v.valid && (!finite(v.index) || v.invalid_reason !== null || Number(v.hr_seconds) < 60 - 1e-9 || Number(v.speed_seconds) < 60 - 1e-9)) ||
...
export function parseTrainabilityIndex(v: unknown): TrainabilityIndex | null {
  ...
  if (!isRecord(v) || v.schema_version !== "trainability-index-v1" ||
    typeof v.model_version !== "string" || typeof v.comparison_key !== "string" ||
    !isRecord(v.source_versions) || !Object.values(v.source_versions).every(x => typeof x === "string") ||
    !nullableNumber(v.hrmax_bpm) || !Array.isArray(v.zone_bounds_bpm) || v.zone_bounds_bpm.length !== 6 || !v.zone_bounds_bpm.every(finite) ||
    !Array.isArray(v.general_range_percent) || v.general_range_percent.join() !== "75,92" ||
    v.minimum_seconds !== 60 || v.minimum_grade_pct !== -3 || v.minimum_activity_seconds !== 420 || !nullableNumber(v.activity_duration_s) ||
...
export function parseTrainabilityHistory(v: unknown): TrainabilityHistory {
  ...
  const activities = v.activities.map(row => {
    ...
    return { ...row, index: parseTrainabilityIndex(row.index) } as unknown as IndexActivity;
  });
...
export const invalidLabel = (reason: string | null) => ({
  HR_TIME_BELOW_60S: "Под 60 сек HRmod", SPEED_TIME_BELOW_60S: "Под 60 сек скорости",
