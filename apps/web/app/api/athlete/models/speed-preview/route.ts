...
export async function GET(request:Request) {
  try {
    ...
    const mode=params.get("test_mode");
    if(mode!==null&&!['STRICT','EXPLORATORY'].includes(mode)||!/^act_[a-f0-9]{32}$/.test(ref)|| (start===null)!==(duration===null) ||
    ...
    if(mode!==null)query.test_mode=mode;
