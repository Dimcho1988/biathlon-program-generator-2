...
export default async function ActivitiesPage({ searchParams }: { searchParams: Promise<{ start?: string; end?: string; sync?: string }> }) {
  ...
  return <main className="activities-page">
    <header className="activities-hero">
      ...
      <div className="activities-title"><div><p className="eyebrow">Реално завършена работа</p><h1>Активности</h1><p>Тренировките ти, подредени в календар. Отвори активност за подробен анализ.</p></div><span>{calendar.activities.length} активности · {calendar.period_start} — {calendar.period_end}{calendar.revision ? ` · версия ${calendar.revision}` : ""}</span></div>
    </header>
