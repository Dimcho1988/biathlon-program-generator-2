...
import { getActivityCalendar } from "../../lib/api";
...
export default async function ActivitiesPage({ searchParams }: { searchParams: Promise<{ start?: string; end?: string }> }) {
  ...
  let calendar;
  try { calendar = await getActivityCalendar(athleteAlias ?? undefined, start, end); }
  catch (error) { return <ErrorState message={error instanceof Error ? error.message : "Календарът временно не е достъпен."} retryAvailable />; }
  ...
  return <main className="activities-page">
    ...
    <ActivityCalendarView calendar={calendar} />
  </main>;
}
