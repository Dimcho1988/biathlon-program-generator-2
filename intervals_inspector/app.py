...
from intervals_inspector.pipeline import run_activity_pipeline
...
def _run_activity_normalizer(
    ...
    *,
    ...
    """Run the reusable pipeline and retain aggregate diagnostics only."""
    ...
    with tabs[4]:
        ...
        if isinstance(stream_quality, Mapping):
            ...
            if isinstance(normalizer_summary, Mapping):
                activity_metadata = normalizer_summary.get("activity_metadata")
                if isinstance(activity_metadata, Mapping):
                    st.subheader("Безопасни metadata за активността")
                    _render_table([dict(activity_metadata)], "Няма безопасни metadata.")

                canonical_input = normalizer_summary.get("canonical_model_input")
                if isinstance(canonical_input, Mapping):
                    st.subheader("Каноничен моделeн вход")
                    _render_table(
                        [dict(canonical_input)],
                        "Каноничният вход още не е създаден.",
                    )
                ...
                if isinstance(history, Mapping):
                    ...
                    _render_table(
                        ...
                        "Няма налична предходна история.",
                    )
                    for warning in history.get("warnings", []):
                        st.warning(str(warning))

                model_status = normalizer_summary.get("model_status")
                if isinstance(model_status, Mapping):
                    status = str(model_status.get("status") or "")
                    reason = str(model_status.get("reason") or "")
                    if status == "not_run":
                        st.error(f"Моделът не е изпълнен: {reason}")
                    elif status == "limited":
                        st.warning(f"Ограничен моделeн резултат: {reason}")
                    elif reason:
                        st.success(reason)
...
def _disconnect(*, preserve_host_state: bool = False) -> None:
    _clear_callback_query()
    if preserve_host_state:
        inspector_prefixes = (
            "_intervals_",
            "_inspector_",
            "_pilot_",
...
def _render_inspector(*, integrated: bool = False) -> None:
    if integrated:
        st.title("onFlows — реални данни и shadow диагностика")
        st.warning(
            "Отделен read-only диагностичен режим. Реалните данни не променят "
            "синтетичното демо, не се записват в Supabase и остават само в паметта."
        )
    else:
    ...
    if st.button(
        ...
        _disconnect(preserve_host_state=integrated)


def render_integrated_page() -> None:
    """Render the inspector inside the root demonstrator."""

    _render_inspector(integrated=True)
