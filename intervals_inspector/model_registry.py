...
REGISTRY_VERSION = "shadow-model-registry-v2-qref-tref"
...
PARAMETER_TEMPLATES: dict[str, dict[str, Any]] = {
    ...
    "spill_threshold_fraction": _definition(
        ...
        "Дял от ефективния Tref, над който директният Qref създава съседен spillover.",
        "X_z = max(0, Qref_z - threshold_z × Tref_effective,z)",
        "% от Tref",
        interpretation="По-нисък праг активира spillover по-рано; 50% означава праг при половината Tref.",
        inputs=("Qref_z", "tref_effective"),
    ...
    "tref_min": _definition(
        ...
        "Минималната допустима седмична Qref референция.",
        "min_effective,z = tref_min,z",
        "референтни минути/седмица",
    ...
    "tref_max": _definition(
        ...
        "Максималната допустима седмична Qref референция.",
        "max_effective,z = tref_max,z",
        "референтни минути/седмица",
    ...
    "bounds_factor": _definition(
        ...
        "c_z",
        "Наследен коефициент на Tref границите",
        "Фиксиран е на 1.0 за съвместимост; новият Tref използва директно min/max границите.",
        "c_z = 1.0",
        "без единица",
        interpretation="1.0 означава, че няма допълнително мащабиране на границите.",
        dependencies=("tref_effective",),
        limitations=("Само за четене; не участва като свободен параметър в новия Tref.",),
...
RESULT_DEFINITIONS: dict[str, dict[str, Any]] = {
    "result.t": _definition(
        ...
        inputs=("нормализиран HR поток", "HR граници"), dependencies=("Q_z", "Qref_z"),
    ...
    "result.q": _definition(
        ...
        inputs=("T_z", "W_low", "W_high", "p"), dependencies=("cascade", "E_z"),
        limitations=("Зависи от качеството на HR и калибрацията на профила.",),
    ),
    "result.qref": _definition(
        "result.qref", "Qref_z", "Референтно претеглено време", "Същият централен W(HR), нормализиран към горната граница за Z1–Z4 и към долната граница за Z5.",
        "Qref_z = ∫ W(HR)/W(HR_high,z) dt; Z5: ∫ W(HR)/W(HR_low,5) dt", "референтни минути",
        interpretation="За Z1–Z4 Qref не надвишава реалното време; за Z5 е поне реалното време. Qref определя Tref и директния spillover праг.",
        inputs=("централен W(HR)", "HR граници"), dependencies=("Tref", "direct ratio", "spillover"),
        limitations=("При липсващ или неизползваем HR Qref е нула и не се публикува валиден моделeн резултат.",),
    ),
    "result.direct_ratio": _definition(
        "result.direct_ratio", "Qref/Tref", "Директен дял от референтния капацитет", "Диагностичното отношение на директния Qref към използвания Tref.",
        "direct_ratio_z = Qref_z / Tref_effective,z", "дял",
        interpretation="0.50 е директният праг за вторична зона; стойността не включва cascade или входящ spillover.",
        inputs=("Qref_z", "tref_effective"), dependencies=("spillover", "диагностично планиране"),
    ...
    "result.spillover": _definition(
        ...
        "X=max(0,Qref-threshold×Tref); Spill=rate×X", "еквивалентни минути",
        interpretation="Нула означава, че няма съседно превишение; по-висока стойност означава по-силен надпрагов съседен стимул.",
        inputs=("Qref_z", "tref_effective", "spill rates"), dependencies=("E_z",),
    ...
    "result.tref_raw": _definition(
        "result.tref_raw", "tref_raw", "Сурова историческа седмична референция", "При 7–39 завършени календарни дни е 7/N по сумата на дневния Qref; при 40 дни използва точно предходните 40. При 0–6 дни идва от избрания профил.",
        "H_N,z = (7/N) × Σ Qref_day,z; H_40,z = (7/40) × Σ Qref_day,z", "референтни минути/седмица",
        ...
        inputs=("предходен дневен Qref_z", "profile level"), dependencies=("tref_effective", "spillover"),
        limitations=("Текущият и бъдещите дни са изключени; календарните дни без активност се броят с Qref=0.",),
    ),
    "result.tref_effective": _definition(
        ...
        "tref_effective = clip(tref_raw, tref_min, tref_max)", "референтни минути/седмица",
        interpretation="Разлика спрямо tref_raw означава активирана долна или горна граница.",
        inputs=("tref_raw", "tref_min", "tref_max"), dependencies=("spillover",),
        limitations=("Ограничението не променя историческите Qref стойности.",),
    ),
    "result.hr_coverage": _definition(
        ...
        inputs=("валидни HR интервали", "активно време"), dependencies=("T_z", "Q_z", "Qref_z", "E_z"),
    ...
    "result.excluded_duration": _definition(
        ...
        inputs=("interval classifications",), dependencies=("активно време", "T_z", "Q_z", "Qref_z"),
...
WARNING_DEFINITIONS: dict[str, dict[str, Any]] = {
    ...
    "warning.incomplete_history": _definition(
        ...
        interpretation="При 0–6 дни Tref идва от профила; при 7–39 дни се използва видима provisional Qref екстраполация.", dependencies=("tref_raw",),
        limitations=("Почивен ден е Qref=0 само когато календарният прозорец е зареден успешно.",),
    ),
    "warning.low_hr_coverage": _definition(
        "warning.low_hr_coverage", "Ниско HR покритие", "Недостатъчно покритие с валиден HR",
        "Част от активното време не участва в T_z, Q_z и Qref_z.", "coverage < 80%", "%",
        interpretation="Резултатите вероятно подценяват реалния зонален товар.", dependencies=("T_z", "Q_z", "Qref_z", "E_z"),
