...
реално време, канонично приравнено време, времево претеглен среден HR, средна
стойност на минутата, cascade/spillover, `E_z` и bounded 40-дневен Tref.
Аеробният Tref е `7 × mean(E)` за предходните до 40 завършени календарни дни,
ограничен в експертните граници за всяка зона; при липса на история се използва
горната граница. Резултатите не се записват обратно в Intervals.

## Единен силов компонент в облачната интеграция

Първата облачна версия разпознава само недвусмислени силови activity types
(`WeightTraining` и консервативни семантични варианти) и ги обединява в един
компонент `STR`. Използва се записаната продължителност с предпочитание към
`icu_recording_time`; една реална минута е една приравнена STR минута
(`k = 1.0`). Не се изисква ръчен силов подтип. Общи типове като `Workout`,
`Crossfit` и `HighIntensityIntervalTraining` не се прекласифицират без
еднозначен provider етикет. HR потокът на разпозната силова активност не
участва в Z1–Z5, което предотвратява двойното отчитане. Подробните силови
разновидности остават запазени за следваща, отделно валидирана версия.
