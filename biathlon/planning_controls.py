    ...
    for cycle in controls["cycles"]:
        ...
        if left <= day <= right:
            state.update(kind=cycle["kind"], name=cycle["name"], accents=cycle["accents"],
                         ...
                         mesocycle_id=left.isoformat(), mesocycle_start=left.isoformat(), mesocycle_end=right.isoformat())
...
def goals(profile, state, actual_base, legacy, *, limited, taper_factor):
    ...
    for z in COMPONENTS:
        ...
        if state["kind"] == "RECOVERY" and accent and z not in manual:
            target = min(base["c40"]*7*.9, max(0., 7*(min(1., state["maintenance_index"])*(base["b50"]+base["c40"])-base["b50"])))
        target *= taper_factor
